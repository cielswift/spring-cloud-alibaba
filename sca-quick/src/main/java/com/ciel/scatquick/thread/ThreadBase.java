...
public class ThreadBase {
    public static void main(String[] args) throws InterruptedException {
        ...
        new Thread(() -> { while (true) { g.produce(); }
        }, "PRODUCE1").start();

        new Thread(() -> { while (true) { g.produce(); }
        }, "PRODUCE2").start();

        new Thread(() -> { while (true) { g.consume(); }
        }, "CONSUME1").start();
    ...
    public static class Goods {
        ...
        private AtomicBoolean flag = new AtomicBoolean(false); //信号灯
        ...
        public void produce() {
            ...
            while (flag.get()) {  //当线程被等待的时候,也会释放锁,其他线程就可以进来;

                //wait() 是 Object 类的方法，会释放对象锁，并让出 CPU 资源。
                //只能在 synchronized 下使用，使用 notify() 或 notiftAll() 唤醒。
